...
exports.ViewCustomer=function(req,res){
    (async()=>{
        const customer_id=req.params.id;
        const viewcustomer=await client.query("select firstname,lastname,gender,mobile_no,email,address,image,area,city from customer where customer_id=$1",[customer_id],(error,responce)=>{
            if(error){
                 res.status(401).json(error);
            }
            res.status(200).json(
