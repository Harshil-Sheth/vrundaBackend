...
exports.ViewOrder=function(req,res){
    (async()=>{
        ...
        const placeorder=await client.query('select placeorder_id,sub_servicename,price,time_duration,order_date,order_status from placeorder p left join subservices s on s.subservice_id =p.subservice_id where customer_id=$1',[customer_id],(error,response)=>{
            if(error){
                return res.status(401).json(error);
            }
            res.status(200).json(
